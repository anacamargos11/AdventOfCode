...
if __name__ == '__main__':
    ...
    x_init = tuple((m.position[0],m.velocity[0]) for m in moons)
    y_init = tuple((m.position[1],m.velocity[1]) for m in moons)
    z_init = tuple((m.position[2],m.velocity[2]) for m in moons)

    x_t, y_t, z_t = tuple(), tuple(), tuple()

    # simulating the system
    t = 0
    while (x_t == x_init and y_t == y_init and z_t == z_init)==False:
        t += 1
        Moon.accelerate(moons)
        Moon.walk(moons)
        x_t = tuple((m.position[0],m.velocity[0]) for m in moons)
        y_t = tuple((m.position[1],m.velocity[1]) for m in moons)
        z_t = tuple((m.position[2],m.velocity[2]) for m in moons)
